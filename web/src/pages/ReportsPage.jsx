...
export default function ReportsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const nextReports = body.reports || body.data?.reports || [];
  ...
  return <div className="flex max-w-[1600px] flex-col gap-5 mx-auto select-none">
    ...
    <div className="rounded-xl border border-soc-border bg-soc-surface p-4"><h3 className="mb-3 border-b border-soc-border pb-2 text-xs font-mono font-bold uppercase tracking-wider text-soc-text">Incident Filings Queue</h3>
      ...
      {!loading && error && <div className="mb-3 flex items-center justify-between rounded-lg border border-soc-warning/40 bg-soc-warning/5 p-3 text-sm text-soc-muted"><span>Service temporarily unavailable. Reconnecting automatically. {lastUpdated ? `Last successful update: ${new Date(lastUpdated).toLocaleString()}` : 'No successful update yet.'}</span><button onClick={refresh} className="ml-3 rounded border border-soc-border px-3 py-1 text-xs text-soc-text">Retry</button></div>}
      {!loading && !error && reports.length === 0 && <p className="text-sm text-soc-muted">No investigations yet.</p>}
      <div className="space-y-2 font-mono text-xs">{reports.map((report) => <div key={report.report_id} className="flex items-center justify-between rounded-lg border border-soc-border bg-soc-bg p-3"><div className="flex items-center gap-3"><FileText className="h-5 w-5 text-soc-danger" /><div><div className="font-bold text-soc-text">{report.report_id}</div><div className="text-[11px] text-soc-muted">Transaction: {report.txn_id} · Amount: INR {Number(report.amount || 0).toLocaleString('en-IN')} · Score: {report.score ?? '—'}</div></div></div><button onClick={() => download(report).catch((exception) => setError(exception.message))} className="rounded border border-soc-border bg-soc-panel px-3 py-1 text-xs text-soc-text hover:border-soc-primary"><Download className="mr-1 inline h-3 w-3" />Download PDF</button></div>)}</div>
    </div>
  </div>;
}
